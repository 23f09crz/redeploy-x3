        ...
        try:
            if home_team == team_name:
                if home_score == 0:
                    ...
                    color = 'yellow'
                corner_kicks_home = next((s['value'] for s in statistics[fixture['id']][0]['statistics'] if s['type'] == "Corner Kicks"), 0)
                corner_kicks_away = next((s['value'] for s in statistics[fixture['id']][1]['statistics'] if s['type'] == "Corner Kicks"), 0)
                st.markdown(
            ...
            if home_team == team_name:
                if home_score == 0:
                    ...
                    color = 'yellow'
                st.markdown(f"{date}: <span style='color:{color}'><b>{home_team} {home_score}</b></span> - {away_score} {away_team}", unsafe_allow_html=True)
            else:
                if away_score == 0:
                    color = 'red'
                elif away_score > home_score:
                    ...
                    color = 'yellow'
                st.markdown(f"{date}: {home_team} {home_score} - <span style='color:{color}'><b>{away_score} {away_team}</b></span>", unsafe_allow_html=True)
